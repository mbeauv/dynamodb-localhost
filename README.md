...
`npm install --save dynamodb-localhost-2`
...
```
var dynamodbLocal = require("dynamodb-localhost-2");
...
{ port : 8000, /* Port to listen on. Default: 8000 */
  ...
  dbPath : '<mypath>/', /* The directory where DynamoDB will write its database file. If you do not specify this option, the file will be written to the current directory. Note that you cannot specify both dbPath and inMemory at once. For the path, current working directory is <projectroot>/node_modules/dynamodb-localhost-2/dynamob. For example to create <projectroot>/node_modules/dynamodb-localhost-2/dynamob/<mypath> you should specify '<mypath>/' with a forwardslash at the end. */
...
Please contribute to the original project instead of this one.  Thanks.
